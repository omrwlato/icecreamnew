...
// File contracts/distribution/MasterNode.sol
...
contract MasterNode {
    ...
    IERC20 public constant CREAM = IERC20(0x3b248CEfA87F836a4e6f6d6c9b42991b88Dc1d58);
    uint256[] public tierAllocPoints = [1 ether, 2.1 ether, 4.41 ether, 9.261 ether, 19.4481 ether]; //circulation
    uint256[] public tierAmounts = [500 ether, 1000 ether, 2000 ether, 4000 ether, 8000 ether]; //node tiers / price per token
    struct User {
    ...
    function claimTreasuryRewards() external {
        if (treasury_rewards > 0) {
            CREAM.safeTransfer(dev, treasury_rewards);
            treasury_rewards = 0;
        }   
    }
    ...
    function create(uint256 nodeTier, uint256 numNodes) external {
        ...
        require(CREAM.balanceOf(_sender) >= tierPrice, "Insufficient balance");
        require(CREAM.allowance(_sender, address(this)) >= tierPrice, "Insufficient allowance");
        CREAM.safeTransferFrom(_sender, address(this), tierPrice);
    ...
    function claim() public {
        ...
        if (_rewards > 0) {
            ...
            IERC20(CREAM).safeTransfer(_sender, _rewards);
    ...
    function getRewardDrip() public view returns (uint256) {
        if (lastDripTime < block.timestamp) {
            uint256 poolBalance = getCreamBalancePool();
    ...
    function getCreamBalance() public view returns (uint256) {
        return IERC20(CREAM).balanceOf(address(this));
    }

     function getCreamBalancePool() public view returns (uint256) {
        return getCreamBalance().sub(total_rewards).sub(treasury_rewards);
    }
